...
# JT test 순서: PFD 오름차순 (fp-50 PFD 가장 낮음, fp-1000 가장 높음)
JT_ORDER=(fp-50 fp-200 fp-1000)

# ── 출력 디렉토리 & 로그 설정 ─────────────────────────────────
_KST_TS="$(date -u -d '+9 hours' +%y%m%d_%H%M%S)"
if [ -n "${RESULT_DIR:-}" ]; then
    OUT_DIR="$RESULT_DIR"
    LOG_FILE="$OUT_DIR/exp_fp_analysis_${_KST_TS}.log"
else
...
# ── 헬퍼 함수 ─────────────────────────────────────────────────
make_job_id() { echo "${1}-draw${DRAW}-rep${3}"; }
