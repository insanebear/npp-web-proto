#!/usr/bin/env bash
# Bayesian (BBN) 전체 배포 스크립트
# Docker 이미지, Task Definition, Lambda 함수를 순서대로 배포
# 사용법: ./scripts/deploy-bbn-all.sh
...
echo "=========================================="
echo "Bayesian (BBN) 전체 배포 프로세스"
echo "=========================================="
echo ""

# 1. Docker 이미지 빌드 및 ECR 푸시
echo "[1/3] Building and pushing Docker image to ECR..."
bash scripts/deploy-bbn-docker.sh
echo ""

# 2. ECS Task Definition 등록
echo "[2/3] Registering ECS Task Definition..."
bash scripts/deploy-bbn-task-definition.sh
echo ""

# 3. Lambda 함수 배포
echo "[3/4] Deploying Lambda function..."
bash scripts/deploy-bbn-lambda.sh
echo ""

# 4. Lambda 함수 환경 변수 설정
echo "[4/4] Setting Lambda function environment variables..."
bash scripts/set-bbn-lambda-env.sh
echo ""
