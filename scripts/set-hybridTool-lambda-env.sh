...
# 사용법: ./scripts/set-hybridTool-lambda-env.sh
# 디버그 모드: DEBUG=1 ./scripts/set-hybridTool-lambda-env.sh
...
for FUNCTION_NAME in "${LAMBDA_FUNCTIONS[@]}"; do
  ...
  if ! aws lambda get-function \
    ...
    echo "     (Deploy the function first using scripts/deploy-hybridTool-lambda.sh)"
    echo ""
    continue
  fi
