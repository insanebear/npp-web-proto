...
export function getCodeKey(sectionLabel: string, fieldLabel: string): string | null {
  const section = labelToCode[sectionLabel];
  if (!section) return null;
  return section[fieldLabel] || null;
}

export function getLabelForCode(sectionLabel: string, codeKey: string): string | null {
  const section = labelToCode[sectionLabel];
  if (!section) return null;
  for (const [label, code] of Object.entries(section)) {
    if (code === codeKey) return label;
  }
  return null;
}
